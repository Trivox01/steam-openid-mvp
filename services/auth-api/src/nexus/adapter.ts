/**
 * Platform adapter boundary — BACKEND-OWNED runtime contract.
 *
 * TRUST BOUNDARY: a provider adapter that resolves provider credentials is
 * BACKEND-ONLY. It must never execute in React/Tauri. The flow is strictly:
 *
 ...
 * backend-owned contract; it does NOT implement any adapter. Genuinely shared
 * result DTOs may be hoisted into src/domain/nexus later if the desktop ever
 * needs them; in Phase 1 nothing desktop-side imports this module.
 */
...
import type { DisconnectPlan } from "./identity.ts";
