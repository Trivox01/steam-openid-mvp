...
export class InMemoryToolRepository implements ToolRepository {
  ...
  async create(input:ToolMutation,actor:string){if([...this.tools.values()].some(x=>x.slug===input.slug))throw new ToolError("TOOL_SLUG_CONFLICT");const now=new Date().toISOString();const tool=await this.materialize({id:randomUUID(),...input,createdAt:now,updatedAt:now});this.tools.set(tool.id,tool);for(const badge of tool.badges)this.audit("tool.badge_assigned",tool.id,actor,{badgeId:badge.id});this.audit("tool.created",tool.id,actor);return tool;}
  async update(id:string,input:ToolMutation,actor:string){const current=this.tools.get(id);if(!current)throw new ToolError("TOOL_NOT_FOUND");if([...this.tools.values()].some(x=>x.id!==id&&x.slug===input.slug))throw new ToolError("TOOL_SLUG_CONFLICT");const next=await this.materialize({...current,...input,updatedAt:new Date().toISOString()});this.tools.set(id,next);for(const badge of next.badges.filter(x=>!current.badges.some(y=>y.id===x.id)))this.audit("tool.badge_assigned",id,actor,{badgeId:badge.id});for(const badge of current.badges.filter(x=>!next.badges.some(y=>y.id===x.id)))this.audit("tool.badge_removed",id,actor,{badgeId:badge.id});this.audit("tool.updated",id,actor);return next;}
