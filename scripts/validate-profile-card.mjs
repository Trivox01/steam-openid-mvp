...
assert.match(css, /\.profile-badge-tooltip-portal\{[^}]*z-index:100/, "portal tooltip layers above the profile popover");
assert.match(css, /\.profile-card-popover\{[^}]*width:390px/, "desktop profile card width remains stable");
assert.match(css, /\.profile-card__banner\{[^}]*height:112px/, "banner uses the recovered compact height");
assert.match(css, /\.profile-card__avatar\{[^}]*width:80px;height:80px/, "avatar uses the recovered 80px size");
