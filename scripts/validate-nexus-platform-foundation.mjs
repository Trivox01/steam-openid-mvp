...
assert.equal(
  migrations.filter((name) => /^021_/.test(name)).length,
  1,
  "Phase 3A must ship exactly one migration 021"
);
assert.ok(
  migrations.includes(catalogMigrationName),
  `Phase 3A migration must be named exactly ${catalogMigrationName}`
);
const catalogMigration = fs.readFileSync(
  path.join(migrationsDir, catalogMigrationName),
  "utf8"
);
const catalogMigrationSql = catalogMigration
  .split("\n")
  .filter((line) => !line.trim().startsWith("--"))
  .join("\n");
for (const table of [
  ...
  assert.match(
    ...
    `migration 021 must create ${table}`
  );
}
assert.match(
  catalogMigrationSql,
  /CREATE UNIQUE INDEX IF NOT EXISTS platform_games_identity_uniq\s+ON platform_games \(provider, provider_game_id\)/i,
  "migration 021 must enforce the provider-game identity"
);
assert.match(
  catalogMigrationSql,
  /CREATE UNIQUE INDEX IF NOT EXISTS user_game_ownership_uniq\s+ON user_game_ownership \(linked_account_id, platform_game_id\)/i,
  "migration 021 must enforce ownership identity"
);
const ownershipMigrationTable = catalogMigrationSql.match(
  /CREATE TABLE IF NOT EXISTS user_game_ownership \(([\s\S]*?)\n\);/i
);
assert.ok(ownershipMigrationTable, "migration 021 must define user_game_ownership");
assert.doesNotMatch(
  ownershipMigrationTable[1],
  /\buser_id\b/i,
  "migration 021 ownership must derive the user through linked_account_id"
);
assert.doesNotMatch(
  ownershipMigrationTable[1],
  /\bprovider\b/i,
  "migration 021 ownership must derive provider through platform_games"
);
assert.match(
  ownershipMigrationTable[1],
  /CONSTRAINT user_game_ownership_playtime_truth CHECK \(\s*playtime_known OR playtime_minutes IS NULL\s*\)/i,
  "migration 021 must preserve unknown-playtime truth"
);
assert.match(
  catalogMigrationSql,
  /CREATE TRIGGER user_game_ownership_provider_match[\s\S]*BEFORE INSERT OR UPDATE OF linked_account_id, platform_game_id[\s\S]*EXECUTE FUNCTION nexus_enforce_ownership_provider_match\(\)/i,
  "migration 021 must enforce provider consistency in the database"
);
const platformMigrationTable = catalogMigrationSql.match(
  /CREATE TABLE IF NOT EXISTS platform_games \(([\s\S]*?)\n\);/i
);
assert.ok(platformMigrationTable, "migration 021 must define platform_games");
assert.match(
  platformMigrationTable[1],
  /canonical_game_id\s+uuid REFERENCES canonical_games\(id\) ON DELETE RESTRICT/i,
  "migration 021 canonical links must use ON DELETE RESTRICT"
);
assert.match(
  platformMigrationTable[1],
  /canonical_mapping_method\s+text CHECK \(canonical_mapping_method IN\s*\(\s*'provider_verified','editorial_verified'\s*\)\)/i,
  "migration 021 must accept only provider/editorial verified mappings"
);
assert.match(
  platformMigrationTable[1],
  /canonical_game_id IS NOT NULL[\s\S]*canonical_mapping_method IS NOT NULL[\s\S]*canonical_verified_by IS NOT NULL[\s\S]*canonical_verified_at IS NOT NULL/i,
  "migration 021 verified mappings must carry canonical id, method, verifier and timestamp"
);
assert.doesNotMatch(
  catalogMigrationSql,
  /provider_credentials|platform_achievements|user_achievement_states|access_token|refresh_token|session_token|credential_ref|client_secret|api_key|poll_secret|encryption_key|password/i,
  "migration 021 must contain no credentials, tokens, secrets or achievement persistence"
);
...
assert.ok(
  fs.existsSync(path.join(backendDir, "catalogRepository.ts")),
  "Phase 3A catalogRepository.ts must remain backend-owned"
);
...
for (const file of walkSourceFiles("src")) {
  ...
  assert.doesNotMatch(
    source,
    /services\/auth-api\/src\/nexus\/catalogRepository/,
    `${file} must not import the backend catalog persistence layer`
  );
}
