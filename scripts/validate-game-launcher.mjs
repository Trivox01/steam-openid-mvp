...
assert.match(card,/<GameActionButton/);assert.match(details,/<GameActionButton/);assert.match(button,/event\.stopPropagation\(\)/);assert.match(shiny,/aria-busy/);assert.match(button,/aria-live="polite"/);assert.match(composition,/steam-installation-changed/);assert.match(composition,/replaceIndex\(payload\.index\);void gameLauncher\.installationChanged\(payload\.appId\)/);assert.match(styles,/prefers-reduced-motion:reduce/);assert.match(styles,/forced-colors:active/);assert.match(styles,/nexus-action-shine 7s/);assert.match(styles,/nexus-shiny-button--play[^}]*var\(--action-play\)/);assert.match(styles,/nexus-shiny-button--install[^}]*var\(--action-install\)/);assert.match(styles,/nexus-shiny-button--running[^}]*animation:none/);assert.match(styles,/nexus-shiny-button--warning/);assert.doesNotMatch(`${card}\n${details}\n${button}`,/openUrl\(|steam:\/\/(run|install|store)\//);
