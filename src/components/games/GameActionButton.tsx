...
function variant(snapshot:GameLaunchSnapshot):NexusShinyVariant{if(snapshot.actionStatus==="error"||snapshot.availability==="steam_unavailable"||snapshot.availability==="steam_not_installed")return"warning";if(snapshot.availability==="running")return"running";if(snapshot.action==="play")return"play";if(snapshot.action==="install")return"install";if(snapshot.action==="store")return"store";return"neutral"}
