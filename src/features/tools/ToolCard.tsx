...
import type { NexusTool } from "./types";
import { ToolImage } from "./ToolImage";
import { ToolCardRating } from "./ToolRating";
export function ToolCard({ tool, onOpen }: { tool: NexusTool; onOpen: (slug: string) => void }) {
  const { t } = useTranslation();
  return (
    <article className="tool-card">
      <div className="tool-card__cover"><ToolImage src={tool.coverUrl} /></div>
      <div className="tool-card__body">
        ...
        <footer>
          ...
          <button type="button" onClick={() => onOpen(tool.slug)}>{t("tools.viewDetails")}<ArrowRight aria-hidden="true" /></button>
        </footer>
      </div>
    </article>
  );
