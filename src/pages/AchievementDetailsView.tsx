...
export function AchievementDetailsView({ achievementId, onClose, onOpenGame }: { achievementId: AchievementId; onClose: () => void; onOpenGame: (id: GameId) => void }) {
  ...
  if (state.status === "error") return <div className="dialog-backdrop"><div className="dialog-loading"><ErrorView message={t(asyncErrorMessageKey(state.error))} onRetry={state.retry} /></div></div>;
